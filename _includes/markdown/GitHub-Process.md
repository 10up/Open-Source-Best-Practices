...
In order to limit our risk to a GitHub Action updating within our workflows to a compromised version (e.g. [Harden-Runner detection: tj-actions/changed-files action is compromised](https://www.stepsecurity.io/blog/harden-runner-detection-tj-actions-changed-files-action-is-compromised)), all action files will be pinned to a specific commit hash that relates to a specific released version.  For example, see [this PR in ClassifAI](https://github.com/10up/classifai/pull/873) that sets these actions to a specific commit hash and notes the respective version as a comment on each line.
